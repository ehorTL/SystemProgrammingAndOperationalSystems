...
public class Kernel {
    ...
    /**
     * TASK 6
     * @param newUmask - new mask in decimal format user want to be set
     */
    public static short umask(short newUmask) {
        //negative or too big number to represent a mask
        if (0 > newUmask || newUmask > 0777) {
            process.errno = EINVAL; //invalid argument
            return -1;
        }
        ...
        System.out.println("Mask set to " + String.format("%03o", newUmask));
...
//directory in which we want to create a link; file descriptor of the file on which we want to create link;
// name of the new file;
    private static void insertFile(int dir, short newInode, String name) throws Exception {
        int status;
        //newDirectoryEntry - the directory entry into which data should be copied
        ...
        while (true) {
            ...
            if (status < 0) {
                ...
                // if entry do not read, write the new item
 ...
 //pathFrom - existing path, pathTo - new hard-link to existing file
    ...
    public static int chmod(String name, short mode) throws Exception {
        ...
        // only the owner or super user can change mode
        if (!((process.getUid() == 0) || (process.getUid() == indexNode.getUid()))) {
            process.errno = EPERM;
            return -1;
        }
